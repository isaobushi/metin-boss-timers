// Maps a remaining-time fraction (1 = full, 0 = end) to an urgency colour:
...
export function urgencyColor(progress: number): string {
  ...
  if (p >= 0.5) return hex(lerp(ORANGE, CYAN, (p - 0.5) / 0.5)); // upper half: orange -> cyan
  return hex(lerp(RED, ORANGE, p / 0.5)); // lower half: red -> orange
}
